...
- 単一ユーザー認証（あなた専用のメモ帳）
  ...
  - 実装: パスキー（WebAuthn）による生体認証。パスワード不要で指紋/顔認証のみ。
  - セッション: JWT 30 日維持（認証頻度を最小化）。
  - デバイス追加: リンクコード（6 桁）で新しいデバイスを追加可能。
  - 将来拡張: 認証層のみ改修でマルチユーザー対応可能（DB 設計は変更不要）。
- 基本検索とタグフィルタ
...
- 期限（dueAt）・優先度（priority） (Task model に属する属性)
  - AC: メモを Todo として扱う際に期日や優先度を付与し、一覧でソート/フィルタできる。`dueAt` / `priority` は `Task` モデル側の属性です（`remindAt` は MVP では廃止）。
...
- マルチユーザー対応
  - AC: 複数ユーザーがそれぞれ独立したデータを管理できる。
  - 実装: 初回アクセス時に「新規 / 既存」選択 UI、QR コードによるデバイス追加。
